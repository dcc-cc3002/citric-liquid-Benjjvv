 ...
 */
class Norma(var level: Int = 1, var currentStars: Int = 0, var currentWins: Int = 0) {
  ...
  def normaCheck(player: Player): Unit = {
    ...
    if (player.Stars == 10 || player.Wins == 1) {
      player.currentNorma = 2   // Establecer la norma del jugador en 2
    }
    if (player.Stars == 30 || player.Wins == 3) {
      player.currentNorma = 3   // Establecer la norma del jugador en 3
    }
    if (player.Stars == 70 || player.Wins == 6) {
      player.currentNorma = 4   // Establecer la norma del jugador en 4
    }
    if (player.Stars == 120 || player.Wins == 10) {
      player.currentNorma = 5   // Establecer la norma del jugador en 5
    }
    if (player.Stars == 200 || player.Wins == 14) {
      player.currentNorma = 6   // Establecer la norma del jugador en 6
    }
  }
}
